...
import PatientProgressRedux, { PatientActions } from '../Redux/PatientProgressRedux'
...
export function* handleCommands(action) {
  ...
  switch (parsedCommand.command) {
    ...
    case 'store-deepstream-userid':
     ...
     jsonData = JSON.parse(parsedCommand.content);
      console.log('data received is: ',jsonData.total_points);
      yield put({
        type: PatientActions.SET_POINTS,
        participantPoints: jsonData.points
      })
      yield put({
        type: PatientActions.SET_TOTAL_POINTS,
        total_points: jsonData.total_points
      })
      yield put({
        type: PatientActions.SET_LEARNING_BOOL,
        participantLearning: jsonData.learningDone
      })
      yield put({
        type: PatientActions.SET_TRAINING_BOOL,
        participantTraining: jsonData.trainingDone
      })
    break;
    ...
    case 'points-array':
      jsonData = JSON.parse(parsedCommand.content);
      yield put({
        type: PatientActions.SET_POINTS_ARRAY,
        participantPoints: jsonData.points
      })
      break;
    ...
    case 'session-data':
      jsonData = JSON.parse(parsedCommand.content);
      console.log('SESSION IS: ',jsonData.sessionIsOn);
      yield put({
        type: PatientActions.SET_SESSION_BOOL,
        sessionIsOn: jsonData.sessionIsOn
      })
    break;

    case 'events-data':
      jsonData = JSON.parse(parsedCommand.content);
      console.log('Aujourd hui: ',jsonData.day)
      yield put({
        type: PatientActions.SET_DATE,
        systemDay:jsonData.day,
        systemMonth:jsonData.month,
        systemYear:jsonData.year,
        learningTime:jsonData.learningTime,
        trainingTime:jsonData.trainingTime
      })
